...
| <a name="input_container_repo_credentials_secret"></a> [container\_repo\_credentials\_secret](#input\_container\_repo\_credentials\_secret) | Container repository credentials; required when using a private repo. This map currently supports a single key; "credentialsParameter", which should be the ARN of a Secrets Manager's secret holding the credentials | `map(string)` | `{}` | no |
