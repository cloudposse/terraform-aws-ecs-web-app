...
func TestExamplesComplete(t *testing.T) {
	...
	// Verify we're getting back the outputs we expect
	assert.Equal(t, "eg-test-ecs-web-app-alb-access-logs", accessLogsBucketId)
	...
	// Verify we're getting back the outputs we expect
	assert.Contains(t, httpListenerArn, "listener/app/eg-test-ecs-web-app")

	// Run `terraform output` to get the value of an output variable
	targetGroupName := terraform.Output(t, terraformOptions, "target_group_name")
	// Verify we're getting back the outputs we expect
	assert.Equal(t, "eg-test-ecs-web-app-default", targetGroupName)
}
